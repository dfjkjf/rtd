...
在 DDS 通信中，数据的“不可达”通常分为 **SampleLost（样本丢失）** 和 **SampleRejected（样本拒绝）** 两类。前者侧重于传输过程中的遗失，后者侧重于因不满足接收规则而被过滤。

#### 1. SampleLost (样本丢失)

此类场景指数据未能成功交付给 DataReader。

* **Reliable 模式下的物理丢失：**
Reader 接收到 Writer 的 Heartbeat 或 Gap 消息，发现某些序列号（SN）的数据在发送端历史缓存中已被覆盖/清除。Reader 无法通过 NACK 重传机制找回这些包，判定为永久丢失。


* **Best Effort 模式下的空洞检测：**
Reader 接收到一个 SN 远大于当前最大 SN 的新包，通过计算差值发现中间存在未接收到的包。


* **接收端资源超限（Resource Limits）：**
    * **Keep_Last 策略：** 当接收缓存队列已满，为存入新数据，被迫覆盖或丢弃尚未被用户读取的旧数据。
    * **Keep_All 策略：** 当缓存达到上限且无法扩容，导致后续新达到的样本无法进入队列（在某些实现中，这也可能触发 Rejected）。


#### 2. SampleRejected (样本拒绝)

此类场景指数据包已到达 Reader，但因校验、规则或资源限制被拒绝处理。

* **Ownership 所有权限制：**
在 `EXCLUSIVE` 所有权配置下，Reader 收到来自低优先级（Strength 值较低）Writer 的样本。


* **序列号异常与重复：**
    * **Reliable 模式：** 接收到已处理过的重复序列号样本。
    * **Best Effort 模式：** 接收到序列号小于或等于当前最大已接收 SN 的“过期”乱序包。


* **数据完整性错误：**
反序列化失败： 数据包在网络层完整，但 Payload 无法按 IDL 定义正确解析。


* **实例限制（Instance Limits）：**
当实例总数超过了 DataReader 的 `max_instances` 配置，新实例的数据将被拒绝。
