...
1. 版本1：两种编码格式
    ...
    PlAIN_CDR：
    - 适用类型：所有最终和可追加类型，同时支持可选成员；
    - 序列化格式：直接加入数据，后必须自动填充保证下一个消息按4字节偏移。
    PL_CDR：填充，负载长度不包含填充字节
    - 适用类型：可变的聚合类型；
    - 序列化格式：组成参数列表。每个参数之前都有一个参数头，该参数头由一个两字节的参数ID和一个两字节的参数长度组成。一个参数接着一个参数，直到遇到列表终止标记。
        参数长度的值是序列化成员的精确长度。为了使下一个参数ID相对于前一个参数ID从4字节偏移处开始，可以添加填充字节。

2. 版本2：三种编码格式

    PlAIN_CDR2：
    - 适用类型：所有最终类型，同时支持可选成员；
    - 序列化格式：与PlAIN_CDR相同，不同之处在于 INT64、UINT64、FLOAT64 和 FLOAT128 的偏移量是按 4 字节对齐的。
    DELIMITED_CDR：
    - 适用类型：所有可追加类型，同时支持可选成员；
    - 序列化格式：与PlAIN_CDR2相同，不同之处在于序列化对象之前，它会先序列化一个 UINT32 分隔符头（字节序和长度）。
    PL_CDR2：填充，负载长度不包含填充字节
    - 适用类型：可变的聚合类型；
    - 序列化格式：组成参数列表。与DELIMITED_CDR相同，序列化参数之前，它会先序列化一个 UINT32 分隔符头（字节序和长度）。
        每个参数之前都有一个参数头，该参数头由一个两字节的参数ID和一个两字节的参数长度组成。一个参数接着一个参数，直到遇到列表终止标记。
        参数长度的值是序列化成员的精确长度。为了使下一个参数ID相对于前一个参数ID从4字节偏移处开始，可以添加填充字节。
